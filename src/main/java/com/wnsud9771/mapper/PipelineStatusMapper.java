...
public interface PipelineStatusMapper {
	...
	@Select("SELECT CASE " +
            ...
            "   THEN true ELSE false END AS allFiltersConsumersTrue " +
            "FROM pipelines p " +
            "JOIN campaign_topic ct ON p.id = ct.pipelines_id " +
            "JOIN format_topic fmt ON ct.id = fmt.campaign_topic_id " +
            "JOIN filter_topic ft ON fmt.id = ft.format_topic_id " +
